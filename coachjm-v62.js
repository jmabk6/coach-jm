...
function programDays(){
 ...
 try{
   ...
   const movableSlots=base.filter(d=>d.kind!=="done" && !(programWeekOffset===0 && d.dateISO<localISO())).map(d=>d._slot);
   const movable=base.filter(d=>d.kind!=="done" && !(programWeekOffset===0 && d.dateISO<localISO()));
...
function saveProgramOrder(days){
  const keys=days.filter(d=>d.kind!=="done" && !(programWeekOffset===0 && d.dateISO<localISO())).map(d=>d.id||d.name);
...
function bindProgramDrag(){
  ...
  function swapData(from,to){
    ...
    if(a<0 || b<0) return;
    if(days[a].kind==="done" || days[b].kind==="done") return;

    const slotA={
      dow:days[a].dow,n:days[a].n,status:days[a].status,
      _slot:days[a]._slot,dateISO:days[a].dateISO
    };
    const slotB={
      dow:days[b].dow,n:days[b].n,status:days[b].status,
      _slot:days[b]._slot,dateISO:days[b].dateISO
    };
