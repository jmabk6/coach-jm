...
function bindProgramDrag(){
  ...
  let active=false, longPress=false, fromSlot=null, dragCard=null, placeholder=null;
  let touchId=null, offsetX=0, offsetY=0, previewTargetSlot=null;
  let pressX=0,pressY=0,pressTimer=null;
  const LONG_PRESS_MS=450;

  function clearTimer(){ if(pressTimer){clearTimeout(pressTimer);pressTimer=null;} }
  function findTouch(list){return [...list].find(t=>t.identifier===touchId);}

  function nearestTarget(clientY){
    let best=null,bestDist=Infinity;
    for(const row of document.querySelectorAll(".program-day-final")){
      ...
      if(dist<bestDist){bestDist=dist;best={row,slot};}
    }
    return best;
  }
  ...
  function updateTarget(y){
    clearPreview();
    const target=nearestTarget(y);
    if(target){
      previewTargetSlot=target.slot;
      target.row.classList.add("swap-preview");
    }
  }

  function startDrag(t,handle){
    ...
    Object.assign(card.style,{
      ...
      zIndex:"99999",pointerEvents:"none",transform:"none"
    });
  ...
  function moveCard(x,y){
    if(!dragCard)return;
    dragCard.style.left=`${x-offsetX}px`;
  ...
  function restore(){
    ...
    if(dragCard){
      ...
      ["position","left","top","width","height","margin","z-index","pointer-events","transform"].forEach(p=>dragCard.style.removeProperty(p));
    }
    if(placeholder&&dragCard)placeholder.replaceWith(dragCard);
    placeholder=null;
  }
  ...
  function swap(from,to){
    ...
    const a=days.findIndex(d=>d._slot===from), b=days.findIndex(d=>d._slot===to);
    if(a<0||b<0||days[a].kind==="done"||days[b].kind==="done")return;

    const slotA={dow:days[a].dow,n:days[a].n,status:days[a].status,_slot:days[a]._slot,dateISO:days[a].dateISO};
    const slotB={dow:days[b].dow,n:days[b].n,status:days[b].status,_slot:days[b]._slot,dateISO:days[b].dateISO};
  ...
  function reset(){
    clearTimer(); active=false;longPress=false;fromSlot=null;dragCard=null;
    placeholder=null;touchId=null;previewTargetSlot=null;
  }
  ...
  window.addEventListener("touchmove",e=>{
    ...
    if(!longPress && moved>10){
      clearTimer(); // scrolling/movement before long press => no menu, no drag
      return;
    }
    if(longPress){
      e.preventDefault();
  ...
  window.addEventListener("touchend",e=>{
    ...
    if(longPress){
      moveCard(t.clientX,t.clientY);
      const from=fromSlot,to=previewTargetSlot;
      restore();
      swap(from,to);
      reset();
      render("program");
      ...
      const slot=fromSlot;
      reset();
      openProgramContextMenu(slot);
    }
  },{passive:false,capture:true,signal});

  window.addEventListener("touchcancel",e=>{
    clearTimer();
    if(longPress)restore();
    reset();
  },{passive:false,capture:true,signal});
  ...
  handles.forEach(handle=>{
    ...
    handle.addEventListener("touchstart",e=>{
      ...
      longPress=false;active=false;

      pressTimer=setTimeout(()=>{
        const current=[...e.touches].find(x=>x.identifier===touchId)||t;
        startDrag(current,handle);
      },LONG_PRESS_MS);
...
function openProgramContextMenu(slot){
  ...
  document.body.insertAdjacentHTML("beforeend",`
    ...
    <div class="program-context-menu" role="dialog" aria-modal="true">
      ...
      ${isRest
        ? `<button type="button" data-program-action="add">＋ Ajouter une séance</button>`
        : `<button type="button" data-program-action="modify">✎ Modifier la séance</button>
  ...
  document.querySelectorAll("[data-program-action]").forEach(btn=>btn.addEventListener("click",()=>{
    ...
    if(action==="add"){
      /* Le créateur existant est réutilisé. Le jour choisi reste mémorisé
         dans programContextSlot via sessionStorage pour le raccordement final. */
   ...
   <div class="program-list program-list-final">
     ${days.map(d=>`
       <div class="program-day program-day-final" data-slot="${d._slot}">
         <div class="datebox ${d.dateISO===localISO()?'today':''}">
           <span>${d.dow}</span>
           <b>${d.n}</b>
         </div>
         ...
         <div class="card workout-card workout-card-final ${d.kind==='done'?'done':''}"
           ...
           ${d.historyId
             ...
             : `<button class="drag-handle" type="button" aria-label="Options ${d.name}" data-slot="${d._slot}"><i></i><i></i><i></i></button>`}
         </div>
       </div>`).join("")}
   </div>
