...
function bindProgramDrag(){
  ...
  handles.forEach(handle=>{
    ...
    handle.addEventListener("touchmove",e=>{
      const t=[...e.changedTouches].find(x=>x.identifier===activeTouchId);
      if(!t || !dragCard)return;
      e.preventDefault();
      e.stopPropagation();
      moveGhost(t.clientX,t.clientY);
      targetAt(t.clientX,t.clientY);
    },{passive:false});

    handle.addEventListener("touchend",e=>{
      const t=[...e.changedTouches].find(x=>x.identifier===activeTouchId);
      if(!t || !dragCard){cleanup();return;}
      e.preventDefault();
      e.stopPropagation();
