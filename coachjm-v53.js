...
function programView(){
 ...
 return `<section class="page program-page-final">
   ...
   <div class="program-list program-list-final">
     ${days.map(d=>`
       <div class="program-day program-day-final ${d.kind==="empty"?"empty-day":""}" data-slot="${d._slot}">
         ...
         ${d.kind==="empty"
           ? `<div class="empty-workout-slot"><span>Aucune séance réalisée</span></div>`
           : `<div class="card workout-card workout-card-final ${d.kind==='done'?'done':''}" ${d.id?`data-route="workout-${d.id}"`:''} ${d.historyId?`data-history-id="${d.historyId}"`:''}>
               <span class="program-status-dot ${d.kind==='done'?'done':d.kind==='rest'?'rest-dot':d.n===8&&programWeekOffset===0?'today':'planned'}"></span>
   ...
   <button class="secondary program-wide-action ${isPast?"disabled-action":""}" id="addProgramSession" ${isPast?"disabled":""}>＋ Ajouter une séance</button>
