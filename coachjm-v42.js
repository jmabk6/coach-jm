...
async function exportCoachData(){
 try{
   ...
   const payload={
     ...
     activeSession:localStorage.getItem(ACTIVE_SESSION_KEY)
   };
   ...
   a.href=url;a.download=`coach-jm-sauvegarde-${stamp}.json`;
   document.body.appendChild(a);a.click();a.remove();
   setTimeout(()=>URL.revokeObjectURL(url),1000);
   alert("Sauvegarde Coach JM créée.");
 }catch(e){
   console.error(e);alert("Impossible de créer la sauvegarde.");
 }
}
async function importCoachData(file){
 try{
   ...
   for(const s of data.workoutSessions) await saveWorkoutSession(s);
   if(data.activeSession) localStorage.setItem(ACTIVE_SESSION_KEY,data.activeSession);
   historyCache=await getAllWorkoutSessions();
...
function render(route){
 ...
 bindCatalog(); bindBuilder(); bindLive(); bindSummary(); bindBackup();
