...
const DAYS = [
 ...
 {dow:"VEN",n:11,status:"rest",name:"Repos",meta:"",kind:"rest"},
...
function programDays(){
  ...
  try{
    ...
    movableSlots.forEach((slot,i)=>{
      ...
      base[slot]={...item,dow:DAYS[slot].dow,n:DAYS[slot].n,status:DAYS[slot].status,_slot:slot};
    });
    return base;
  }catch(e){
    return base;
...
function saveProgramOrder(days){
  ...
  localStorage.setItem(PROGRAM_ORDER_KEY,JSON.stringify(keys));
}
 ...
 return `<section class="page program-page-final">
   ...
   <div class="week-nav compact-week-nav">
     ...
     <div class="week-label">
       <strong>7 — 13 septembre</strong>
       <span>1 réalisée · 5 prévues</span>
     </div>
     <button class="round">›</button>
   </div>
