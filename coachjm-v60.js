...
function bindProgramDrag(){
  /* IMPORTANT : render() recrée l'écran. On détruit donc les anciens listeners
     globaux avant d'en attacher de nouveaux. */
  ...
  function resetVisual(){
    document.querySelectorAll(".workout-card.dragging").forEach(el=>{
      el.classList.remove("dragging","drag-floating");
      el.style.removeProperty("transform");
      el.style.removeProperty("z-index");
      el.style.removeProperty("pointer-events");
    });
    document.querySelectorAll(".program-day.drop-candidate").forEach(el=>el.classList.remove("drop-candidate"));
  }
  ...
  function findNearestSlot(clientY){
    const rows=[...document.querySelectorAll(".program-day-final")];
    let best=null;
    let bestDist=Infinity;

    for(const row of rows){
      const handle=row.querySelector(".drag-handle[data-slot]");
      if(!handle) continue; // séances réalisées/verrouillées
      const r=row.getBoundingClientRect();
      const cy=r.top+r.height/2;
      const dist=Math.abs(clientY-cy);
      if(dist<bestDist){
        bestDist=dist;
        best={row,slot:+handle.dataset.slot};
      }
    }
    return best;
  }
  ...
  function updateTarget(clientY){
    document.querySelectorAll(".program-day.drop-candidate").forEach(el=>el.classList.remove("drop-candidate"));
    const nearest=findNearestSlot(clientY);
    currentTargetSlot=nearest?.slot ?? null;
    /* Pas de bordure rouge sur la destination : juste une très légère indication. */
    if(nearest && nearest.slot!==fromSlot) nearest.row.classList.add("drop-candidate");
  }
  ...
  function onTouchMove(e){
    ...
    /* C'est bien le bloc ORIGINAL qui suit le doigt. */
    if(dragCard){
      dragCard.style.transform=`translate3d(${dx}px,${dy}px,0)`;
      dragCard.style.zIndex="9999";
    }

    updateTarget(t.clientY);
  }
  ...
  function onTouchEnd(e){
    ...
    if(t) updateTarget(t.clientY);

    const to=currentTargetSlot;
    const from=fromSlot;

    /* On nettoie TOUJOURS le rouge avant de rerender. */
    cleanup();
  ...
  /* Écoute au niveau WINDOW + capture :
     iOS ne peut plus "perdre" le relâchement lorsque le doigt sort de la poignée. */
  window.addEventListener("touchmove",onTouchMove,{passive:false,capture:true,signal});
  window.addEventListener("touchend",onTouchEnd,{passive:false,capture:true,signal});
  window.addEventListener("touchcancel",onTouchCancel,{passive:false,capture:true,signal});
  window.addEventListener("blur",()=>{if(active){cleanup();render("program");}},{signal});
  ...
  handles.forEach(handle=>{
    ...
    handle.addEventListener("touchstart",e=>{
      ...
      dragCard=handle.closest(".workout-card");
      if(!dragCard) return;

      startX=t.clientX;
      startY=t.clientY;
      currentTargetSlot=fromSlot;
      active=true;

      dragCard.classList.add("dragging","drag-floating");
      dragCard.style.zIndex="9999";
