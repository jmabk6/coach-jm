...
function localISO(d=TODAY){
 return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
}
function frenchLongDate(d=TODAY){
 const days=["dimanche","lundi","mardi","mercredi","jeudi","vendredi","samedi"];
 const months=["janvier","février","mars","avril","mai","juin","juillet","août","septembre","octobre","novembre","décembre"];
 return `${days[d.getDay()][0].toUpperCase()+days[d.getDay()].slice(1)} ${d.getDate()} ${months[d.getMonth()]}`;
}
const BASE_WEEK_START = new Date(2026,8,7); // 7 septembre 2026, lundi
let programWeekOffset = 0;

const BASE_WEEK_TEMPLATE = [
 {dow:"LUN",status:"planned",name:"Cardio léger + mobilité",meta:"≈ 42 min · Cardio",kind:"planned"},
 {dow:"MAR",status:"planned",name:"Muscu A — Jambes",meta:"≈ 55 min · Salle · 5 exercices",kind:"planned",id:"muscu-a"},
...
function findSessionByISO(iso){
  return historyCache.find(x=>{
...
function baseDaysForCurrentWeek(){
 ...
 return BASE_WEEK_TEMPLATE.map((template,i)=>{
   const dd=datePlusDays(s,i);
   const iso=`${dd.getFullYear()}-${String(dd.getMonth()+1).padStart(2,"0")}-${String(dd.getDate()).padStart(2,"0")}`;

   /* Semaine passée : uniquement réel ou Repos. */
   if(programWeekOffset<0){
     const session=findSessionByISO(iso);
     if(session){
       return {
         dow:template.dow,n:dd.getDate(),dateISO:iso,_slot:i,
         kind:"done",status:"done",
         name:session.templateName||"Séance réalisée",
         meta:`${ft(session.elapsedSeconds||0)}`,
         historyId:session.id
       };
     }
     return {
       dow:template.dow,n:dd.getDate(),dateISO:iso,_slot:i,
       kind:"rest",status:"rest",name:"Repos",meta:"",historyId:null
     };
   }

   /* Semaine actuelle :
      - jours avant aujourd'hui : réel ou Repos ;
      - aujourd'hui : réel si séance enregistrée, sinon planning ;
      - jours futurs : planning. */
   if(programWeekOffset===0){
   ...
   return {...template,n:dd.getDate(),dateISO:iso,_slot:i};
 });
}
const PROGRAM_ORDER_KEY="coachjm_program_slots_v3";

function sessionKey(d){
  return d?.id || d?.name || "";
}

function programStorageKey(){
  return `${PROGRAM_ORDER_KEY}_${programWeekOffset}`;
}

function movableProgramDays(base){
  return base.filter(d=>
    d.kind!=="done" &&
    !(programWeekOffset===0 && d.dateISO<localISO())
  );
}

function programDays(){
  const base=baseDaysForCurrentWeek();
  if(programWeekOffset<0) return base;

  try{
    const saved=JSON.parse(localStorage.getItem(programStorageKey())||"null");
    if(!saved || typeof saved!=="object" || Array.isArray(saved)) return base;

    const movable=movableProgramDays(base);
    const byKey=new Map(movable.map(d=>[sessionKey(d),d]));

    for(const d of movable){
      const key=saved[String(d._slot)];
      if(!key) continue;
      ...
      base[d._slot]={...item,...slotInfo};
    }

    return base;
  }catch(e){
    console.error("Programme restore",e);
    return base;
  }
}

function saveProgramSlots(days){
  const map={};

  for(const d of movableProgramDays(days)){
    map[String(d._slot)]=sessionKey(d);
  }

  localStorage.setItem(programStorageKey(),JSON.stringify(map));
}
function bindProgramDrag(){
  if(programDragController) programDragController.abort();
  programDragController=new AbortController();
  const {signal}=programDragController;
  ...
  let previewTargetSlot=null;

  function findTouch(list){
    return [...list].find(t=>t.identifier===touchId);
  }

  function getMovableRowBySlot(slot){
    return document.querySelector(`.program-day-final[data-slot="${slot}"]`);
  }

  function nearestTarget(clientY){
    let best=null, bestDist=Infinity;
    const rows=[...document.querySelectorAll(".program-day-final")];

    for(const row of rows){
      const h=row.querySelector(".drag-handle[data-slot]");
      if(!h) continue;

      const slot=+h.dataset.slot;
  ...
  function clearPreview(){
    document.querySelectorAll(".program-day-final.swap-preview").forEach(el=>el.classList.remove("swap-preview"));
    previewTargetSlot=null;
  }

  function showSwapPreview(slot){
    if(slot===previewTargetSlot) return;
    clearPreview();

    const sourceRow=getMovableRowBySlot(fromSlot);
    const targetRow=getMovableRowBySlot(slot);
    if(!sourceRow || !targetRow) return;

    targetRow.classList.add("swap-preview");
    previewTargetSlot=slot;
  }
  ...
  function moveCard(clientX,clientY){
    ...
    const target=nearestTarget(clientY);
    if(target) showSwapPreview(target.slot);
    else clearPreview();
  }
  ...
  function restoreCard(){
    ...
    if(dragCard){
      dragCard.classList.remove("dragging","drag-fixed");
      ["position","left","top","width","height","margin","z-index","pointer-events","transform"].forEach(p=>{
        dragCard.style.removeProperty(p);
      });
    }
  ...
  function swapData(from,to){
    ...
    const a=days.findIndex(d=>d._slot===from);
    const b=days.findIndex(d=>d._slot===to);

    if(a<0 || b<0) return false;
    if(days[a].kind==="done" || days[b].kind==="done") return false;

    const slotA={
      dow:days[a].dow,
      n:days[a].n,
      status:days[a].status,
      _slot:days[a]._slot,
      dateISO:days[a].dateISO
    };
    const slotB={
      dow:days[b].dow,
      n:days[b].n,
      status:days[b].status,
      _slot:days[b]._slot,
      dateISO:days[b].dateISO
    };
    ...
    /* Sauvegarde PAR CASE : la cible prend réellement A,
       l'origine prend réellement B. */
    saveProgramSlots(days);
    return true;
  }
  ...
  function onEnd(e,cancelled=false){
    ...
    if(t)moveCard(t.clientX,t.clientY);

    const targetSlot=cancelled?null:previewTargetSlot;
    const sourceSlot=fromSlot;

    /* Le bloc revient d'abord proprement dans le DOM, puis on échange les 2 séances. */
    restoreCard();

    if(targetSlot!==null){
      swapData(sourceSlot,targetSlot);
    }
  ...
  window.addEventListener("touchmove",onMove,{passive:false,capture:true,signal});
  window.addEventListener("touchend",e=>onEnd(e,false),{passive:false,capture:true,signal});
  window.addEventListener("touchcancel",e=>onEnd(e,true),{passive:false,capture:true,signal});
  ...
  handles.forEach(handle=>{
    handle.addEventListener("contextmenu",e=>e.preventDefault(),{signal});
    ...
    handle.addEventListener("touchstart",e=>{
      ...
      placeholder.className="program-drag-placeholder";
      ...
      card.classList.add("dragging","drag-fixed");
...
function closeProgramContextMenu(){
  document.querySelector(".program-context-backdrop")?.remove();
  document.querySelector(".program-context-menu")?.remove();
  programContextSlot=null;
}

function setProgramSlotToRest(slot){
  const days=programDays();
  const idx=days.findIndex(d=>d._slot===slot);
  if(idx<0 || days[idx].kind==="done") return;

  const d=days[idx];
  days[idx]={
    dow:d.dow,n:d.n,dateISO:d.dateISO,_slot:d._slot,
    status:"rest",kind:"rest",name:"Repos",meta:""
  };
  saveProgramSlots(days);
}

function openProgramContextMenu(slot){
  closeProgramContextMenu();

  const d=programDays().find(x=>x._slot===slot);
  if(!d)return;

  programContextSlot=slot;
  const isRest=d.kind==="rest" || d.name==="Repos";
  const isDone=d.kind==="done";

  let actions="";
  if(isDone){
    actions=`<button type="button" data-program-action="view-done">Voir la séance réalisée</button>`;
  }else if(isRest){
    actions=`<button type="button" data-program-action="add">＋ Ajouter une séance</button>`;
  }else{
    actions=`<button type="button" data-program-action="modify">✎ Modifier la séance</button>
             <button type="button" data-program-action="rest">○ Mettre en repos</button>`;
  }

  document.body.insertAdjacentHTML("beforeend",`
    <div class="program-context-backdrop"></div>
    <div class="program-context-menu" role="dialog" aria-modal="true">
      <div class="program-context-title">${d.dow} ${d.n}</div>
      ${actions}
      <button type="button" class="context-cancel" data-program-action="cancel">Annuler</button>
    </div>`);

  document.querySelector(".program-context-backdrop")?.addEventListener("click",closeProgramContextMenu);

  document.querySelectorAll("[data-program-action]").forEach(btn=>btn.addEventListener("click",()=>{
    const action=btn.dataset.programAction;
    const selected=programDays().find(x=>x._slot===programContextSlot);
    const slot=programContextSlot;
    closeProgramContextMenu();

    if(action==="cancel")return;

    if(action==="view-done" && selected?.historyId){
    ...
    if(action==="rest"){
      setProgramSlotToRest(slot);
      render("program");
      return;
    }

    if(action==="modify" && selected){
      selectedProgramPlan=selected;
      navigate("planned-program-detail");
      return;
    }

    if(action==="add"){
      sessionStorage.setItem("coachjm_builder_program_slot",String(slot));
      sessionStorage.setItem("coachjm_builder_program_week",String(programWeekOffset));
      navigate("new");
    }
  }));
}

function bindProgramDateMenus(){
  document.querySelectorAll("[data-date-slot]").forEach(el=>{
    el.addEventListener("click",e=>{
      e.preventDefault();
      e.stopPropagation();
      openProgramContextMenu(+el.dataset.dateSlot);
    });
  });
}
...
function todayView(){
 ...
 <div class="topline"><div><h1 class="brand">Coach JM</h1><div class="date">${frenchLongDate()}</div></div><div class="avatar">JM</div></div>
 ...
 <div class="week-strip">${programDays().map(d=>`<div class="day-mini ${(d.dateISO===localISO())?'active':''}"><span class="dow">${d.dow}</span><b>${d.n}</b><i class="dot ${d.kind==='done'?'done':''}"></i></div>`).join("")}</div>
...
function pastWeekDays(){
  ...
  return dows.map((dow,i)=>{
    ...
    const session=historyCache.find(x=>{
      if(!x?.endedAt)return false;
      const d=new Date(x.endedAt);
      const local=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
      return local===iso;
    ...
    if(session){
      return {
        dow,n:dd.getDate(),dateISO:iso,_slot:i,
        kind:"done",
        name:session.templateName||"Séance réalisée",
        meta:`${ft(session.elapsedSeconds||0)}`,
        historyId:session.id
...
function programView(){
 const isPast=programWeekOffset<0;
 const days=isPast?pastWeekDays():programDays();
 ...
 return `<section class="page program-page-final">
   <div class="program-header compact-head">
     <div class="backless">
       <h1>Mon programme</h1>
       <p>${isPast?"Ce qui a réellement été fait.":"Une semaine prévue, adaptable au réel."}</p>
     </div>
   </div>
   ...
   <div class="week-nav compact-week-nav">
     ...
     <div class="week-label">
       ...
       <span>${isPast
         ...
         : `${doneCount} réalisée${doneCount>1?"s":""} · ${plannedCount} prévue${plannedCount>1?"s":""}`}</span>
     </div>
   ...
   <div class="program-list program-list-final">
     ${days.map(d=>`
       <div class="program-day program-day-final" data-slot="${d._slot}">
         <div class="datebox ${d.dateISO===localISO()?'today':''}" data-date-slot="${d._slot}">
           <span>${d.dow}</span>
           <b>${d.n}</b>
         </div>

         <div class="card workout-card workout-card-final ${d.kind==='done'?'done':''}"
              ${d.historyId?`data-history-id="${d.historyId}"`:`data-program-slot="${d._slot}"`}>
           <span class="program-status-dot ${
             d.kind==='done'?'done':
             d.kind==='rest'?'rest-dot':
             d.dateISO===localISO()?'today':'planned'
           }"></span>
           ...
           ${d.historyId
             ...
             : (isPast || (programWeekOffset===0 && d.dateISO<localISO()))
               ? `<div class="drag-handle locked" aria-hidden="true"><i></i><i></i><i></i></div>`
               : `<button class="drag-handle" type="button" aria-label="Déplacer ${d.name}" data-slot="${d._slot}"><i></i><i></i><i></i></button>`}
         </div>
       </div>`).join("")}
   </div>
<button class="secondary program-wide-action history-open" data-route="history">
     ▥ &nbsp; Historique des séances
   </button>
 </section>`;
}
const PROGRAM_PLAN_DETAILS={
 "Muscu A — Jambes":{
   icon:"🦵",subtitle:"Jambes · gainage · mobilité",duration:"55 min",place:"Salle",
   exercises:["Tapis — échauffement","Squat","Leg Press horizontale","Leg Curl","Gainage frontal","Mobilité hanches / ischios"]
 },
 "Muscu B — Tirage":{
   icon:"💪",subtitle:"Dos · biceps · objectif traction",duration:"55 min",place:"Salle",
   exercises:["Tapis — échauffement","Tirage vertical","Rowing assis","Gainage frontal","Mobilité hanches / ischios"]
 },
 "Muscu C — Poussée":{
   icon:"🏋️",subtitle:"Pectoraux · épaules · triceps",duration:"55 min",place:"Salle",
   exercises:["Tapis — échauffement","Chest Press","Poussée épaules","Gainage frontal","Mobilité"]
 },
 "Cardio — Intervalles":{
   icon:"❤️",subtitle:"Tapis · blocs de travail",duration:"35 min",place:"Cardio",
   exercises:["Échauffement","Intervalles progressifs","Retour au calme"]
 },
 "Cardio long":{
   icon:"❤️",subtitle:"Endurance",duration:"50 min",place:"Cardio",
   exercises:["Échauffement","Cardio continu","Retour au calme"]
 },
 "Cardio léger + mobilité":{
   icon:"🚶",subtitle:"Cardio léger · mobilité",duration:"42 min",place:"Cardio",
   exercises:["Cardio léger","Mobilité"]
 },
 "Repos":{
   icon:"◦",subtitle:"Récupération",duration:"—",place:"—",exercises:[]
 }
};
let selectedProgramPlan=null;
let programDragController=null;
let programContextSlot=null;

function plannedProgramDetailView(){
 const d=selectedProgramPlan;
 if(!d)return `<section class="page"><div class="detail-top"><button class="backbtn" data-route="program">‹</button><div class="detail-title"><h1>Séance prévue</h1></div></div></section>`;
 const p=PROGRAM_PLAN_DETAILS[d.name]||{icon:"📋",subtitle:d.meta||"",duration:"—",place:"—",exercises:[]};
 return `<section class="page">
   <div class="detail-top"><button class="backbtn" data-route="program">‹</button><div class="detail-title"><h1>Séance prévue</h1><p>${d.dow} ${d.n} · ${d.name}</p></div></div>
   <div class="card workout-one-card">
     <div class="summary-head"><div class="bigemoji">${p.icon}</div><div><h2>${d.name}</h2><div class="meta">${p.subtitle}</div></div></div>
     <div class="pills"><span class="pill">⏱ ${p.duration}</span><span class="pill">🏋️ ${p.place}</span>${p.exercises.length?`<span class="pill">📋 ${p.exercises.length} exercices</span>`:""}</div>
   </div>
   ${p.exercises.length?`<div class="section-head"><h2>Contenu prévu</h2></div>
   <div class="exercise-list">${p.exercises.map((name,i)=>`<div class="card exercise"><div class="num">${i+1}</div><div><div class="exercise-name">${name}</div></div></div>`).join("")}</div>`:
   `<div class="card note-card"><b>Repos</b><br>Aucune séance prévue.</div>`}
 </section>`;
}
...
function render(route){
 ...
 route==="workout-muscu-a"?workoutDetail("A"):route==="workout-muscu-d"?workoutDetail("D"):route==="planned-program-detail"?plannedProgramDetailView():
 ...
 document.querySelectorAll("[data-route]").forEach(el=>el.addEventListener("click",async ()=>{
   ...
   /* L'onglet Séance ouvre toujours Mes séances.
      Une séance active reste accessible via sa carte "Séance en cours",
      mais ne détourne jamais la navigation principale. */
   navigate(el.dataset.route);
 }));
 bindCatalog(); bindBuilder(); bindLive(); bindSummary(); bindBackup(); bindProgramDrag(); bindProgramControls(); bindProgramDateMenus();
 document.querySelectorAll("[data-history-id]").forEach(el=>el.addEventListener("click",()=>{
   selectedHistoryId=el.dataset.historyId;
   navigate("history-detail");
 }));
 document.querySelectorAll("[data-program-slot]").forEach(el=>el.addEventListener("click",e=>{
   if(e.target.closest(".drag-handle"))return;
   const slot=+el.dataset.programSlot;
   selectedProgramPlan=programDays().find(d=>d._slot===slot)||null;
   navigate("planned-program-detail");
 }));
