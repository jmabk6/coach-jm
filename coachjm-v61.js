...
function bindProgramDrag(){
  ...
  function movableRows(){
    return [...document.querySelectorAll(".program-day-final")].filter(row=>row.querySelector(".drag-handle[data-slot]"));
  }
  ...
  function updateTarget(clientY){
    ...
    for(const row of movableRows()){
      ...
      if(dist<bestDist){
        bestDist=dist;
        best={row,slot:+h.dataset.slot};
      }
    }

    currentTargetSlot=best?.slot ?? null;
    if(best && best.slot!==fromSlot)best.row.classList.add("drop-candidate");
  }
  ...
  function onEnd(e,cancelled=false){
    ...
    const from=fromSlot;
    const to=cancelled?null:currentTargetSlot;

    /* Restore DOM position before data/rerender. */
    cleanupVisual();
  ...
  handles.forEach(handle=>{
    ...
    handle.addEventListener("touchstart",e=>{
      ...
      /* Placeholder keeps the original row height while THE ORIGINAL CARD
         itself becomes position:fixed and follows the finger. */
