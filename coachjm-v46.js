...
const PROGRAM_ORDER_KEY="coachjm_program_order_v1";
function programDays(){
 const base=DAYS.map((d,i)=>({...d,_slot:i}));
 try{
   const saved=JSON.parse(localStorage.getItem(PROGRAM_ORDER_KEY)||"null");
   if(!Array.isArray(saved))return base;
   const plannedSlots=base.filter(d=>d.kind==="planned").map(d=>d._slot);
   const planned=base.filter(d=>d.kind==="planned");
   const byName=new Map(planned.map(d=>[d.name,d]));
   const ordered=saved.map(n=>byName.get(n)).filter(Boolean);
   planned.forEach(d=>{if(!ordered.includes(d))ordered.push(d)});
   plannedSlots.forEach((slot,i)=>{
     const session=ordered[i];
     base[slot]={...session,dow:DAYS[slot].dow,n:DAYS[slot].n,status:DAYS[slot].status,_slot:slot};
   });
   return base;
 }catch(e){return base}
}
function saveProgramOrder(days){
 const names=days.filter(d=>d.kind==="planned").map(d=>d.name);
 localStorage.setItem(PROGRAM_ORDER_KEY,JSON.stringify(names));
}
let programDragFrom=null;
function bindProgramDrag(){
 const cards=[...document.querySelectorAll(".workout-card[data-movable='1']")];
 let timer=null,startX=0,startY=0,dragEl=null;

 function cleanup(){
   clearTimeout(timer);timer=null;
   document.querySelectorAll(".program-day.drag-target,.workout-card.dragging").forEach(x=>x.classList.remove("drag-target","dragging"));
   dragEl=null;programDragFrom=null;
 }
 cards.forEach(card=>{
   card.addEventListener("touchstart",e=>{
     ...
     timer=setTimeout(()=>{
       programDragFrom=+card.dataset.slot;dragEl=card;card.classList.add("dragging");
   ...
   card.addEventListener("touchmove",e=>{
     const t=e.touches[0];
     if(!dragEl){
       if(Math.abs(t.clientX-startX)>12||Math.abs(t.clientY-startY)>12){clearTimeout(timer);timer=null}
       return;
     }
     e.preventDefault();
     document.querySelectorAll(".program-day.drag-target").forEach(x=>x.classList.remove("drag-target"));
     const under=document.elementFromPoint(t.clientX,t.clientY)?.closest(".program-day");
     if(under && under.querySelector(".workout-card[data-movable='1']"))under.classList.add("drag-target");
   },{passive:false});
   card.addEventListener("touchend",e=>{
     clearTimeout(timer);
     if(!dragEl){timer=null;return}
     const t=e.changedTouches[0];
     const target=document.elementFromPoint(t.clientX,t.clientY)?.closest(".program-day");
     const targetCard=target?.querySelector(".workout-card[data-movable='1']");
     if(targetCard){
       const to=+targetCard.dataset.slot;
       const days=programDays();
       const a=days.findIndex(d=>d._slot===programDragFrom);
       const b=days.findIndex(d=>d._slot===to);
       if(a>=0&&b>=0&&days[a].kind==="planned"&&days[b].kind==="planned"){
         ...
         days[a]={...B,...slotA}; days[b]={...A,...slotB};
         saveProgramOrder(days);
       }
     }
     cleanup();render("program");
   },{passive:false});
...
function todayView(){
 ...
 <div class="week-strip">${programDays().map(d=>`<div class="day-mini ${d.n===8?'active':''}"><span class="dow">${d.dow}</span><b>${d.n}</b><i class="dot ${d.kind==='done'?'done':''}"></i></div>`).join("")}</div>
...
function programView(){
 ...
 <div class="program-list compact-program">${days.map(d=>`<div class="program-day" data-slot="${d._slot}">
   <div class="datebox ${d.n===8?'today':''}"><span>${d.dow}</span><b>${d.n}</b></div>
   <div class="card workout-card compact-workout ${d.kind==='done'?'done':''} ${d.kind==='rest'?'rest':''}" ${d.kind==="planned"?`data-movable="1" data-slot="${d._slot}"`:''} ${d.id?`data-route="workout-${d.id}"`:''}>
    <span class="program-status-dot ${d.kind==='done'?'done':d.kind==='rest'?'rest':d.n===8?'today':'planned'}"></span>
    <div class="workout-main"><div class="workout-name">${d.name}</div><div class="workout-meta">${d.meta.replace(" · réalisé","")}</div></div>
    <div class="chev">›</div>
   </div></div>`).join("")}</div>
 <div class="week-actions single-action"><button class="secondary">+ Ajouter</button></div>
 <button class="secondary history-open" data-route="history">Historique des séances</button>
 <div class="hint"><b>Pour déplacer une séance :</b> maintiens le doigt sur une séance prévue, puis glisse-la sur un autre jour prévu. Les deux séances échangent leur place. Une séance réalisée reste à sa date réelle.</div>
 </section>`;
}
...
function render(route){
 ...
 bindCatalog(); bindBuilder(); bindLive(); bindSummary(); bindBackup(); bindProgramDrag();
