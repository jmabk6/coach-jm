...
function bindProgramDrag(){
 ...
 let timer=null,startX=0,startY=0,dragEl=null,fromSlot=null;
 ...
 handles.forEach(handle=>{
   ...
   handle.addEventListener("touchmove",e=>{
     ...
     if(under && under.querySelector(".drag-handle[data-slot]")) under.classList.add("drag-target");
   },{passive:false});
...
function programView(){
 ...
 <div class="program-header compact-head"><div class="backless"><h1>Mon programme</h1><p>Une semaine prévue, adaptable au réel.</p></div></div>
 <div class="week-nav compact-week-nav"><button class="round">‹</button><div class="week-label"><strong>7 — 13 septembre</strong><span>1 réalisée · 5 prévues</span></div><button class="round">›</button></div>

 <div class="program-list program-list-final">${days.map(d=>`<div class="program-day program-day-final" data-slot="${d._slot}">
   <div class="datebox ${d.n===8?'today':''}"><span>${d.dow}</span><b>${d.n}</b></div>
   <div class="card workout-card workout-card-final ${d.kind==='done'?'done':''} ${d.kind==='rest'?'rest':''}" ${d.id?`data-route="workout-${d.id}"`:''}>
     ...
     <div class="workout-main">
       ...
       ${d.kind==="rest"?"":`<div class="workout-meta">${d.meta.replace(" · réalisé","").replace("≈ ","")}</div>`}
     </div>
     ${d.kind==="planned"
 ...
 <button class="secondary program-wide-action">＋ Ajouter une séance</button>
