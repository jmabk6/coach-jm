...
function render(route){
 ...
 route==="live-workout"?liveView():route==="live-workout-d"?liveView():route==="live-complete"?liveDone():route==="workout-summary"?workoutSummary():route==="history"?historyView():route==="history-detail"?historyDetailView():route==="progress"?progressionView():route==="progression"?progressionView():route==="progression-detail"?progressionDetailView():route==="coach"?coachView():route==="builder-info"?builderInfo():route==="builder-template"?builderInfo():route==="builder-exercises"?builderExercises():route==="builder-recap"?builderRecap():route==="exercise-chest-press"?chestPressDetail():
 route.startsWith("exercise-")?genericExerciseDetail(route.replace("exercise-","")):
 `<section class="page"><div class="program-header"><div class="backless"><h1>Plus</h1><p>Réglages et outils Coach JM.</p></div></div><div class="template-list"><div class="card template-row" data-route="coach"><div class="ico">🧠</div><div><h3>Conseils Coach JM</h3><p>Recommandations basées sur tes séances</p></div><div class="chev">›</div></div><div class="card template-row" data-route="history"><div class="ico">🗂️</div><div><h3>Historique</h3><p>Revoir toutes les séances enregistrées</p></div><div class="chev">›</div></div></div></section>`;
 document.querySelectorAll(".nav-item").forEach(b=>{const activeRoute=(route==="coach")?"more":(route==="workout-summary"||route==="history")?"program":(route==="workout-muscu-a"||route==="workout-muscu-d"||route==="live-workout"||route==="live-workout-d"||route==="live-complete")?"sessions":(route==="sessions"||route==="new"||route.startsWith("builder-")||route.startsWith("exercise-")||route==="catalog")?"sessions":route;b.classList.toggle("active",b.dataset.route===activeRoute)});
