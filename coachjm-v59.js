...
function bindProgramDrag(){
  ...
  function clearTargets(){
    document.querySelectorAll(".program-day.drag-target").forEach(el=>el.classList.remove("drag-target"));
  }
  ...
  function resetDrag(){
    ...
    if(dragCard){
      dragCard.classList.remove("dragging","drag-floating");
      dragCard.style.removeProperty("position");
      dragCard.style.removeProperty("left");
      dragCard.style.removeProperty("top");
      dragCard.style.removeProperty("width");
      dragCard.style.removeProperty("height");
      dragCard.style.removeProperty("margin");
      dragCard.style.removeProperty("z-index");
      dragCard.style.removeProperty("pointer-events");
      dragCard.style.removeProperty("transform");
    }
  ...
  function currentTouch(list){
    return [...list].find(t=>t.identifier===touchId);
  }

  function moveOriginal(x,y){
    if(!dragCard || !cardRect)return;
    const dx=x-startX, dy=y-startY;
    dragCard.style.transform=`translate3d(${dx}px,${dy}px,0)`;
  }

  function targetAt(x,y){
    /* Le bloc déplacé ne doit pas intercepter elementFromPoint. */
    if(dragCard)dragCard.style.pointerEvents="none";
    const el=document.elementFromPoint(x,y);
    if(dragCard)dragCard.style.pointerEvents="";

    const target=el?.closest(".program-day");
  ...
  function onMove(e){
    if(!dragging)return;
    const t=currentTouch(e.changedTouches);
    if(!t)return;
    e.preventDefault();
    moveOriginal(t.clientX,t.clientY);
  }
  ...
  function finish(e,cancelled=false){
    ...
    if(!cancelled && t){
      e.preventDefault();
      const hit=targetAt(t.clientX,t.clientY);
      if(hit)to=+hit.targetHandle.dataset.slot;
    }

    if(to!==null && to!==fromSlot){
  ...
  document.addEventListener("touchend",e=>finish(e,false),{passive:false});
  document.addEventListener("touchcancel",e=>finish(e,true),{passive:false});
  ...
  handles.forEach(handle=>{
    ...
    handle.addEventListener("touchstart",e=>{
      ...
      startX=t.clientX;
      startY=t.clientY;
      cardRect=dragCard.getBoundingClientRect();
      dragging=true;

      /* Aucun clone : c'est le bloc original qui devient rouge et suit le doigt. */
