...
function bindProgramDrag(){
  ...
  function resetDrag(){
    dragging=false;
    clearTargets();
    document.querySelectorAll(".workout-card.dragging").forEach(el=>el.classList.remove("dragging"));
    removeGhost();
    fromSlot=null;
    dragCard=null;
    touchId=null;
  ...
  function makeGhost(card,x,y){
    ...
    ghost.classList.add("program-drag-ghost");
    ghost.querySelectorAll("[data-route],[data-history-id],button").forEach(el=>{
      el.removeAttribute("data-route");
      el.removeAttribute("data-history-id");
  ...
  function getTarget(x,y){
    clearTargets();
    const target=document.elementFromPoint(x,y)?.closest(".program-day");
    if(target && target.querySelector(".drag-handle[data-slot]")){
  ...
  /* IMPORTANT :
     touchmove/touchend/touchcancel sont écoutés sur document.
     Donc même si le doigt sort complètement de la poignée ou de la carte,
     le relâchement est toujours reçu et l'état rouge est supprimé. */
  document.addEventListener("touchmove",onDocumentMove,{passive:false});
  document.addEventListener("touchend",onDocumentEnd,{passive:false});
  document.addEventListener("touchcancel",onDocumentCancel,{passive:false});
  ...
  handles.forEach(handle=>{
    ...
    handle.addEventListener("touchstart",e=>{
      ...
      makeGhost(dragCard,t.clientX,t.clientY);
